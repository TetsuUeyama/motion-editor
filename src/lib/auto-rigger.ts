 ...
 * 共通の型・定数・関数は voxel-core.ts から取得する
 *
 * 主要機能:
...
import {
  Scene, Mesh, AbstractMesh, VertexData, Vector3,
} from '@babylonjs/core';

// voxel-core.ts から共通の型・定数・関数をre-export
export {
...
// このファイル内で使用するためimport
import {
  type VoxelEntry, type Vec3, type BoneDef, type BoneFrameData, type MotionClip,
  BONE_DEFS, FACE_DIRS, FACE_VERTS, FACE_NORMALS,
  voxelToViewer, assignVoxelsToBones, addSphereCaps, createUnlitMaterial,
} from '@/lib/voxel-core';
...
// BONE_DEFS は voxel-core.ts からre-export済み
...
// モーションクリップ読み込みは voxel-skeleton.ts の loadMotionClipFromFile を使用
// 後方互換のため loadMotionClip という名前でもre-export
export { loadMotionClipFromFile as loadMotionClip } from '@/lib/voxel-skeleton';
...
// ============================================================
// ボクセル→ボーン割り当て（キャッシュ付き）
// 複数のエクスポート関数が同じvoxels+bonesに対して呼ぶ場合、
// 重い計算を1回だけ実行するためのヘルパー
// ============================================================
function prepareBoneVoxels(voxels: VoxelEntry[], bones: Record<string, Vec3>): Record<string, VoxelEntry[]> {
  const boneVoxels = prepareBoneVoxels(voxels, bones);
  return boneVoxels;
}
